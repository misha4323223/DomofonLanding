...
3. **Site URL**: https://www.obzor71.ru
4. **Кастомная подписка** - после отправки заявки клиенту предлагается включить уведомления
5. **Service Worker** настроен для получения уведомлений
6. **Теги клиента** - телефон, имя, город и адрес автоматически сохраняются

## ⚠️ ВАЖНО - Настройка OneSignal Dashboard:

### Отключите автоматический промпт:
1. Откройте https://dashboard.onesignal.com/
2. Settings → Push & In-App → Web → Permission Prompts
3. **УДАЛИТЕ или ОТКЛЮЧИТЕ** "Push Slide Prompt"
4. Сохраните изменения

**Почему?** Сайт показывает свой промпт после отправки заявки. Автоматический промпт OneSignal будет мешать.
...
3. **Выберите получателей:**
   ...
   - **Вариант B:** "Send to Particular Segment" → **"User Tag"**
     - Выберите тег: **phone** или **name** или **city**
     - Введите значение (например, телефон клиента)
     - Найдёте конкретного клиента!
...
### 📱 Как найти конкретного клиента:

**Способ 1: По телефону**
- New Push → Send to Particular Segment
- Segment: "User Tag"
- Tag: `phone`
- Value: `+79056298708` (телефон клиента)

**Способ 2: По имени**
- Tag: `name`
- Value: `Иван` (имя клиента)

**Способ 3: По городу**
- Tag: `city`
- Value: `Тула` (отправить всем из Тулы)
